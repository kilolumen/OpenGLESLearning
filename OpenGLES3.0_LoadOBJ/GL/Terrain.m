...
@property (nonatomic, strong) GLKTextureInfo *gressTexture;
...
- (id)initWithGLContext:(GLContext *)context heightMap:(UIImage *)image size:(CGSize)terrainSize height:(CGFloat)terrainHeight gress:(GLKTextureInfo *)gressTexture dirt:(GLKTextureInfo *)dirtTexture
{
    ...
    if (self) {
        ...
        self.gressTexture = gressTexture;
...
- (void)buildGeometry {
    ...
    for(int row = 0; row < self.terrainSize.height; ++row){
        ...
        for(int col = 0; col <= self.terrainSize.width; ++col){
            ...
            GLVertex vectex1 = GLVertexMake(position1.x, position1.y, position1.z, normal1.x, normal1.y, normal1.z, col / (GLfloat)self.terrainSize.width * 2, row / (GLfloat)self.terrainSize.height * 2);
            [terrainMeshStrip appendVertex:vectex1];
...
- (void)draw:(GLContext *)glContext {
    ...
    [glContext bindTexture:self.gressTexture to:GL_TEXTURE0 uniformName:@"grassMap"];
