...
#define ST_MOBILE_BODY_KEYPOINTS    0x08000000  ///< 检测人体关键点
...
#define ST_MOBILE_BODY_ACTION1 0x0100000000    /// 龙拳
#define ST_MOBILE_BODY_ACTION2 0x0200000000    /// 一休
#define ST_MOBILE_BODY_ACTION3 0x0400000000    /// 摊手
...
#define ST_MOBILE_HAND_DETECT_FULL        0x001EFF00  ///< 检测所有手势
#define ST_MOBILE_BODY_DETECT_FULL        0x0708000000 ///< 检测body关键点和所有动作
...
typedef struct st_mobile_body_t {
    int id;                     ///< body id
    st_pointf_t *p_key_points;  ///< body关键点
    float * p_key_points_score;          ///< 关键点的置信度[0,1] 值越大，置信度越高.建议用户使用0.15作为置信度阈值.
    int key_points_count;       ///< body关键点个数 目前为14或0
    unsigned long long body_action;   ///< body动作，本版本无效
    float body_action_score;                ///< body动作置信度,本版本无效
...
typedef struct st_mobile_human_action_t {
    ...
    st_mobile_body_t *p_bodys;  ///< 检测到的人体信息
...
#define ST_MOBILE_ENABLE_BODY_KEYPOINTS         0x00001000  ///< 检测人体关键点及动作（动作本版本无效）
#define ST_MOBILE_ENABLE_BODY_ACTION            0x00002000  ///< 检测人体动作
/// 检测模式
...
typedef enum {
    ...
    /// 设置每多少帧检测一次人体关键点. 默认每帧检测一次. 最多每10帧检测一次. 开启隔帧检测后, 只能对拷贝出来的检测结果做镜像.
...
/// @param[in] b_rotate_background 是否旋转背景图片
