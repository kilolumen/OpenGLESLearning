...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    UIImage *srcImg = [self m_imageFromPixelBuffer: [firstInputFramebuffer renderBuffer]];
    
    [self renderToTextureWithVertices:cropSquareVertices textureCoordinates:cropTextureCoordinates];
    
    UIImage *croppedImg = [self m_imageFromPixelBuffer: [outputFramebuffer renderBuffer]];

    [self informTargetsAboutNewFrameAtTime:frameTime];
}

- (UIImage *)m_imageFromPixelBuffer:(CVPixelBufferRef)pixelBufferRef {
