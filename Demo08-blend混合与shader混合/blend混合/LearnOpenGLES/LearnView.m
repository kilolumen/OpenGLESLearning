...
- (void)setupProgram {
    ...
    NSString* fragFile = [[NSBundle mainBundle] pathForResource:@"shaderf8" ofType:@"fsh"];
...
//    GLuint radius = [self.mProgram uniformIndex:@"in_circle_radius"];
//    GLuint times  = [self.mProgram uniformIndex:@"in_zoom_times"];
//    GLuint imageWidth = [self.mProgram uniformIndex:@"imageWidth"];
//    GLuint imageHeight = [self.mProgram uniformIndex:@"imageHeight"];
...
//    uniform sampler2D texDownSample;
    GLuint texSize = [self.mProgram uniformIndex:@"texSize"];
//    GLuint mosaicSize = [self.mProgram uniformIndex:@"mosaicSize"];
    GLuint texture0Uniform = [self.mProgram uniformIndex:@"texture0"];
//    GLuint texture1Uniform = [self.mProgram uniformIndex:@"texDownSample"];
    ...
    //加载纹理
    [self setupFirstTexture:@"meinv"];
//    [self setupSecondTexture:@"shuaige3"];
//    glUniform1f(radius, 100.0);
//    glUniform1f(times, 2.0);
    glUniform2f(texSize, (GLfloat)width, (GLfloat)height);
//    glUniform2f(mosaicSize, 20.0, 20.0);
    glUniform1i(texture0Uniform, 0);
//    glUniform1f(texture1Uniform, 1);
}
...
- (GLuint)setupSecondTexture:(NSString *)fileName {
    ...
    float fw = width, fh = height;
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fw/4, fh/4, 0, GL_RGBA, GL_UNSIGNED_BYTE, spriteData);
