...
@property (nonatomic, strong) NSMutableArray<GLGeometry *>* terrianMeshStrips;
@end
@implementation GLTerrain
- (id)initWithGLContext:(GLContext *)context//OpenGL环境 program shader
              heightMap:(UIImage *)image//高度图 是灰度图只有一种颜色
                   size:(CGSize)terrainSize//长宽
                 height:(GLfloat)terrainHeight//地形图高度
                  grass:(GLKTextureInfo *)gressTexture//草地纹理
                   dirt:(GLKTextureInfo *)dirtTexture//地面纹理
{
    self = [super initWithGLContext:context];//所有的shader操作都在GLContext中
    ...
    if (self) {
        ...
        self.terrianMeshStrips = [NSMutableArray array];
        self.grassTexture = gressTexture;
...
- (GLKVector3)vertexPosition:(int)col
               ...
               bytesPerPixel:(size_t)bytePerPixel
{
    long long offset = (int)(row / self.terrainSize.height * self.heightMap.size.height) * bytesPerRow + (int)(col / self.terrainSize.width * self.heightMap.size.width) * bytePerPixel;
...
- (void)buildGeometry
{
    ...
    for (int row = 0; row < self.terrainSize.height; ++row) {
        ...
        for (int col = 0; col <= self.terrainSize.width; ++col) {
            GLKVector3 position1 = [self vertexPosition:col
                                                    row:row
                                                 buffer:buffer
                                            bytesPerRow:bytesPerRow
                                          bytesPerPixel:bytesPerPixel];
            GLKVector3 normal1 = [self vertexNormal:position1
                                                col:col
                                                row:row
                                             buffer:buffer
                                        bytesPerRow:bytesPerRow
                                      bytesPerPixel:bytesPerPixel];
            GLVertex vertex1 = GLVertexMake(position1.x,
                                            position1.y,
                                            position1.z,
                                            normal1.x,
                                            normal1.y,
                                            normal1.z,
                                            col / (GLfloat)self.terrainSize.width * 2,
                                            row / (GLfloat)self.terrainSize.height * 2);
            
            [terrainMeshStrip appendVertex:vertex1];
            
            GLKVector3 position2 = [self vertexPosition:col
                                                    row:row + 1
                                                 buffer:buffer
                                            bytesPerRow:bytesPerRow
                                          bytesPerPixel:bytesPerPixel];
            GLKVector3 normal2 = [self vertexNormal:position2
                                                col:col
                                                row:row + 1
                                             buffer:buffer
                                        bytesPerRow:bytesPerRow
                                      bytesPerPixel:bytesPerPixel];
            GLVertex vertex2 = GLVertexMake(position2.x,
                                            position2.y,
                                            position2.z,
                                            normal2.x,
                                            normal2.y,
                                            normal2.z,
                                            col / (GLfloat)self.terrainSize.width * 2,
                                            (row + 1) / (GLfloat)self.terrainSize.height * 2) ;
            
            [terrainMeshStrip appendVertex:vertex2];
        }
        [self.terrianMeshStrips addObject:terrainMeshStrip];
    }
    free(buffer);
}
...
- (void)draw:(GLContext *)glContext
{
    ...
    for (GLGeometry *geometry in self.terrianMeshStrips) {
        [glContext drawGeometry:geometry];
    }
}

- (GLubyte *)dataFromImage:(UIImage *)image
{
