...
+ (id)contextWithVertexShaderPath:(NSString *)vertexShaderPath fragmentShaderPath:(NSString *)fragmentShaderPath
{
    ...
    NSString *fragmentShaderCount = [NSString stringWithContentsOfFile:fragmentShaderPath encoding:NSUTF8StringEncoding error:nil];
    return [[GLContext alloc] initWithVertexShader:vertexShaderContent fragmentShader:fragmentShaderCount];
}
...
- (id)initWithVertexShader:(NSString *)vertexShader fragmentShader:(NSString *)fragmentShader
{
    ...
    if (self) {
        [self setupVertexShader:vertexShader fragmentShader:fragmentShader];
    }
    return self;
}
...
- (void)bindAttribs:(GLfloat *)trangleData
{
    ...
    GLuint texCoordAttribLocation = glGetAttribLocation(program, "texCoord");
    ...
    glVertexAttribPointer(positionAttribLocation, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (char *)trangleData);
...
- (void)drawTrianglesWithVBO:(GLuint)vbo vertexCount:(GLuint)vertexCount
{
    ...
    glDrawArrays(GL_TRIANGLES, 0, vbo);
}
...
{
    glActiveTexture(textureChannel);
    glBindTexture(GL_TEXTURE_2D, textureInto.name);
    GLuint textureID = (GLuint)textureChannel - (GLuint)GL_TEXTURE0;
    [self setUniform1i:uniformName value:textureID];
}
...
bool createProgram(const char *vertexShader,
...
bool linkProgram(GLuint program)
{
    ...
    if (logLenght > 0) {
        ...
        glGetProgramInfoLog(program, logLenght, &logLenght, log);
